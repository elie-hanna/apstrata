...
/*
 * This script implements the account creation procedure (creation of an account + user profile).
 * It is meant to be called from the "widgets.Registration.verifyAccount" script that needs to invoke the  
 * handleAccountCreation() function to start the account creation procedure.
 ...
 *     {  
 	"metadata" : {
		...
		"status": "success"
		"result": {
			"aps_authKey": THE_ACCOUNT_AUTH_KEY,
			"aps_authSecret": THE_ACCOUNT_SECRET
			}
		}
	}
 * (2)) in case of failure:
 	{
 	"metadata": {
		"requestId": "24d4b72c-ae1f-48e3-b7f9-5d20f8c0da39",
		"status": "success",
		"statusCode": "200"
		},
	"result": {
...
function handleAccountCreation(request, user, configuration, apsdb, logLevel){
	...
	try {		
		...
		var reqStore = request.parameters["profileStore"];
		var zStore = reqStore  ? reqStore : configuration.defaultProfileStore;
			
	
		// 1) Create the profile
		var createProfileResponse = createUserProfile(request, user, configuration, zStore, apsdb, logLevel);
								
		if (createProfileResponse.metadata.status == "failure"){
		
			// if profile creation failed, delete the user
			deleteUser(request, user, configuration, apsdb, logLevel);
			throw createProfileResponse.metadata.errorDetail;			
		}		
				
		// 2) If successfuly created the profile create an account
		var createAccountResponse = createAccount(request, user, configuration, apsdb, logLevel);		
			
		if (createAccountResponse.status == "failure"){
			
			deleteUser(request, user, configuration, apsdb, logLevel);
			deleteUserProfile(request, user, configuration, zStore, apsdb, logLevel);
			throw createAccountResponse.errorDetail;
		}
...
function createAccount(request, user, configuration, apsdb, logLevel) {
	
	// Create an account for that user
	var accountParams = {
			"accountId": encodeURIComponent(user.result.documents[0].email),
			"email": encodeURIComponent("_" + user.result.documents[0].email),
			"name": encodeURIComponent(user.result.documents[0].name),
	...
	accountParams["apsws.time"] = ts;
	accountParams["apsws.authSig"] = hash;
	accountParams["apsws.authMode"] = "simple";		
	
	var zurl = submitUrl + "?apsws.time=" + ts + "&apsws.authSig=" + hash + "&apsws.authMode=simple";
	zurl = zurl + "&accountId=" + accountParams["accountId"] + "&email=" + accountParams["email"] + "&name=" + accountParams["name"] + "&provisioningAccount=false&apsws.responseType=json";
		
	var res = apsdb.callHttp(zurl , 'GET', null, null, null, null, false, null, false, false);
	res = parseResultOrThrowError(res);
		
	return res;
}

/*
 * Parses the returned response in JSON format.
 * @param resp : the response received from the callHttp
 */
function parseResultOrThrowError(resp) {
	
	var zBody = resp["body"];	
	var pContent = JSON.parse(zBody );
	...
	if (zStatus == "failure") {
		...
		return {status : "failure" , errorDetail : zErrorDetail};
	}
...
/*
 * Parses the returned response in HTTP format. 
 * @param resp : the response received from the callHttp
 */
function parseHttpResponse(response) {
	var statusBTag = "\<status\>";
	var statusETag = "\</status\>";
	var statusBegin = response.indexOf(statusBTag) + statusBTag.length;
	var statusEnd = response.indexOf(statusETag);
	var status = response.substring(statusBegin, statusEnd);
	if (status == "success") {
		return {status : "success"};
	}else {
		var errorBTag = "\<errorDetail\>";
		var errorETag = "\</errorDetail\>";
		var errorBegin = response.indexOf(errorBTag) + errorBTag.length;
		var errorEnd = response.indexOf(errorETag);
		var error = response.substring(errorBegin, errorEnd);
		var cDataBTag = "\<![CDATA[";
		var cDataETag = "]]\>";
		var cDataBegin = error.indexOf(cDataBTag) + cDataBTag.length;
		var cDataEnd = error.indexOf(cDataETag);
		error = error.substring(cDataBegin, cDataEnd);
		return ( { status : "failure", errorDetail : error });
	}		
}

function createUserProfile(request, user, configuration, store, apsdb, logLevel) {
		
	var profileDocKey = user.result.documents[0].login.replace("@", "_at_");	
	var createProfileParams = {
		"apsdb.documentKey": profileDocKey + "Profile",
		"apsdb.store": store,
		"company": user.result.documents[0].company,
		"webSite": user.result.documents[0].webSite,
		"jobTitle": user.result.documents[0].jobTitle,
		"phone": user.result.documents[0].phone
	}
			
	return apsdb.callApi("SaveDocument", createProfileParams , null);
}

function deleteUserProfile(request, user, configuration, store, apsdb, logLevel) {
	
	var profileDocKey = user.result.documents[0].login.replace("@", "_at_");
	var deleteProfileParams = {
		"apsdb.documentKey": profileDocKey + "Profile",
		"apsdb.store": store
	}
	
	return apsdb.callApi("DeleteDocument", deleteProfileParams, null);	
}

function deleteUser(request, user, configuration, apsdb, logLevel) {

	var params = {
		login: user.result.documents[0].login,				
	}
