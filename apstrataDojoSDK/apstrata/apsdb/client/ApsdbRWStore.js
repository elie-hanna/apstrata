 ...
 * *****************************************************************************
 */
dojo.provide("apstrata.apsdb.client.ApsdbRWStore");
...
dojo.declare("apstrata.apsdb.client.ApsdbRWStore",
[apstrata.util.logger.Logger],
{
    ...
    constructor: function(/* apstrata.apsdb.client.Connection */ connection, queryParams) {
...
//	self.log("It is in the store", this._itemsMap[something[this.connection._KEY_APSDB_ID]])
    ...
    _assertIsItem: function(/* item */ item){
	    ...
	    if(!this.isItem(item)){ 
this.log("assert is item item", item)
		    throw new Error("apstrata.apsdb.client.ItemFileReadStore: Invalid item argument.");
	    }
    },
    ...
    _assertIsReady: function(){
	    ...
	    //		The item to test for being contained by the store.
	    if(!this._isReady){ 
		    throw new Error("apstrata.apsdb.client.ItemFileReadStore: Store not ready.");
	    }
    },
    ...
    newItem: function(/* Object? */ keywordArgs, /*Object?*/ parentInfo){
        ...
        if ((typeof keywordArgs) != "object") throw new Error('apstrata.apsdb.client.ItemFileWriteStore: keywordArgs is not an object');
        var newItem = {}
...
self.log("*********** save completed")
    ...
    deleteItem: function(/* item */ item){
        ...
        self._assertIsItem(item);

        // Mark the new item as deleted
        self._setItemStatus(item, self._FLAG_ITEM_DELETED)

        // Put the item in the dirty stack so it can be updated when save() is invoked
        self._itemsDirty.push(item) 


		self.save({onComplete: function() {
...
console.debug("deleted *************")
    ...
    _assertIsString: function(s) {
        if (s == undefined) throw new Error('apstrata.apsdb.client.ItemFileWriteStore: attribute is undefined');
        if ((typeof s) != "string") throw new Error('apstrata.apsdb.client.ItemFileWriteStore: attribute is not a string');
    },
    ...
    _deleteDocument: function(item, handleResult, handleError) {
        var self = this;
        var op = new apstrata.apsdb.client.DeleteDocument(connection);
    ...
    _processUpdates: function(queue, finishHandler) {
		...
		//
		// Process the top item in the queue which will trigger the rest 
		//
		if (queue.length>0) {
